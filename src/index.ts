...
function buildMcpServer(): McpServer {
  ...
  server.tool(
    ...
    {},
    async () => {
      const data = await whoopGet("/developer/v2/user/profile/basic");
  ...
  server.tool(
    ...
    "Get recent Whoop recovery scores (HRV, resting heart rate, recovery %). Optionally bound by date range.",
    {
      ...
      limit: z.number().optional().describe("Max records to return (default 10, max 25)"),
    },
    async ({ start, end, limit }) => {
      const data = await whoopGet("/developer/v2/recovery", {
        start,
        end,
  ...
  server.tool(
    ...
    async ({ start, end, limit }) => {
      const data = await whoopGet("/developer/v2/activity/sleep", {
        start,
        end,
        limit: String(limit ?? 10),
      });
  ...
  server.tool(
    ...
    async ({ start, end, limit }) => {
      const data = await whoopGet("/developer/v2/activity/workout", {
        start,
        end,
        limit: String(limit ?? 10),
      });
  ...
  server.tool(
    ...
    async ({ start, end, limit }) => {
      const data = await whoopGet("/developer/v2/cycle", {
        start,
        end,
        limit: String(limit ?? 10),
      });
  ...
  server.tool(
    ...
    {},
    async () => {
      const data = await whoopGet("/developer/v2/user/measurement/body");
      return { content: [{ type: "text", text: JSON.stringify(data, null, 2) }] };
    }
  );
